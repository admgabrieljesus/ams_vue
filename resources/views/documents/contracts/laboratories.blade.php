...
<body>
    <div>
        ...
        <h1>CONVÊNIO PARA ATENDIMENTOS NA ÁREA DE LABORATÓRIO – PROFISSIONAIS PESSOA JURÍDICA</h1>
        ...
        <p>e do outro lado, a empresa {{ $request['name'] }}), nome de Fantasia {{ $request['name'] }}, CNPJ
            {{ $request['cnpj'] }}, sediado na Rua {{ $request['address'] }}, N° {{ $request['number'] }},
            {{ $request['complement'] }}, bairro {{ $request['district'] }},
            {{ $request['city'] }}/{{ $request['state'] }}, CEP: {{ $request['zipcode'] }}, telefone
            ({{ $request['ddd'] }}) {{ $request['phone'] }}, email {{ $request['email'] }}, neste instrumento
        ...
        <p><b>4.5</b> – Consta como anexo a este Convênio a relação das especialidades ofertadas;</p>
        <p><b>4.6</b> – Consta como anexo a este convênio a relação dos profissionais do <b>CONVENIADO</b>, nome, CNPJ,
        ...
        <p>Selecionar o laboratório, agendar conforme disponibilidade, apresentar a carteira de associado ou dependente
        ...
        <p>OBS: A apresentação da cópia de credenciamento válido e atualizado pela Assistência Médica da Petrobras no
        ...
        <p>6.1.6 – Cópia de credenciamento na Assistência Médica da Petrobras no estado da Bahia;</p>
